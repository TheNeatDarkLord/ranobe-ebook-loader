...
export async function loadDom(url: string, signal: AbortSignal, method = 'GET', body?: any, validate?: (doc: Document) => boolean) {
    for (let attempt = 0; ; ++attempt) {
        ...
        // Нарвались на антибот. Ожидание блок НЕ снимает (проверено) — снять можно
        // только пройдя проверку заново в браузере. Поэтому пара коротких попыток
        // (вдруг разовый сбой) и сразу отдаём управление, не долбя сайт.
        if (attempt >= 2) {
            throw {
                name: 'BotWallError',
                message: 'Сайт временно заблокировал загрузку (антибот, лимит ~30 запросов за раз).\nУже скачанные главы СОХРАНЕНЫ. Чтобы продолжить:\n1) обновите страницу (F5) и пройдите проверку «Я не робот»;\n2) снова нажмите «Скачать» — загрузка продолжится с места, без повтора.\nДля больших книг это нужно повторить несколько раз.\nЕсли блокирует сразу на 0% — подождите 20-30 минут не заходя на сайт: IP временно в бане из-за частых попыток.'
            };
        }
        await delay(2000 + attempt * 1000, signal);
    }
}
